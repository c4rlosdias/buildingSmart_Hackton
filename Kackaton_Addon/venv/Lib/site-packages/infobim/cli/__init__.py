...
def main():
    ...
    if cmd == "init":
        ...
        if os.path.exists(is_engine_installed_script):
            if sys.platform == "win32":
                # On Windows, skip the bash-based engine check (bash scripts are not natively available)
                pass
            else:
    ...
    if cmd == "run":
        ...
        print("")
        if sys.platform != "win32" and os.path.exists(print_log_script):
            subprocess.run(["bash", print_log_script, "ERROR", f"The '{cmd}' command was not found."], check=False)
        else:
            print(f"[ERROR] The '{cmd}' command was not found.")
        print("")
        print_help()
        sys.exit(1)
