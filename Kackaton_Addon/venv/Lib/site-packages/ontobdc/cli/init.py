...
def log(level, message, *args):
    ...
    if sys.platform != "win32" and os.path.exists(log_script):
...
def message_box(color, title_type, title_text, message):
    ...
    if sys.platform != "win32" and os.path.exists(msg_box_script):
        subprocess.run(["bash", msg_box_script, color, title_type, title_text, message], check=False)
    else:
        # Fallback
        print(f"[{title_type}] {title_text}: {message}")
...
def _confirm_context_creation(path: str) -> bool:
    ...
    # On Windows, termios/tty are not available — fall back to plain input
